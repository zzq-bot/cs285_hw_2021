...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        assert obs_unnormalized.shape[1:] == obs_mean.shape, f"{obs_unnormalized.shape}, {obs_mean.shape}"
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction, _ = self(obs_unnormalized=obs, acs_unnormalized=acs, obs_mean=data_statistics['obs_mean'],
                            obs_std=data_statistics['obs_std'], acs_mean=data_statistics['acs_mean'],
                            acs_std=data_statistics['acs_std'], delta_mean=data_statistics['delta_mean'],
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        unnormalized_delta = next_observations - observations
        target = normalize(unnormalized_delta, data_statistics['delta_mean'], data_statistics['delta_std'])
        target = ptu.from_numpy(target)

        _, pred_delta = self(obs_unnormalized=observations, acs_unnormalized=actions, obs_mean=data_statistics['obs_mean'],
                            obs_std=data_statistics['obs_std'], acs_mean=data_statistics['acs_mean'],
                            acs_std=data_statistics['acs_std'], delta_mean=data_statistics['delta_mean'],
                            delta_std=data_statistics['delta_std'])
        loss = self.loss(input=pred_delta, target=target)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
