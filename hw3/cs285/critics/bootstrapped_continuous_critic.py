...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            #compute target
            v_s_prime = self(next_ob_no) * (1-terminal_n)
            assert v_s_prime.shape == reward_n.shape, f"{v_s_prime.shape}, {reward_n.shape}"
            target = reward_n + self.gamma * v_s_prime
            target = target.detach()
            for _ in range(self.num_grad_steps_per_target_update):
                pred_value = self(ob_no)
                loss = self.loss(input=pred_value, target=target)
                loss_log += loss.item()
