...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            for rewards in rewards_list:
                list_of_discounted_returns = self._discounted_return(rewards)
                q_values += list_of_discounted_returns

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            for rewards in rewards_list:
                list_of_discounted_cumsums = self._discounted_cumsum(rewards)
                q_values += list_of_discounted_cumsums
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            if self.gae_lambda is not None:
                ...
                assert advantages.shape == values.shape, \
                    f"advantages.shape: {advantages.shape}, values.shape: {values.shape}"
                assert rews.shape[0] == batch_size, \
                    f"rews.shape: {rews.shape}, batch_size: {batch_size}"
        ...
        if self.standardize_advantages:
            ...
            advantages = utils.normalize_batch(advantages)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        T = len(rewards)
        list_of_discounted_cumsums = [0] * (T+1)
        for i, r in enumerate(reversed(rewards)):
            list_of_discounted_cumsums[T-i-1] = r + self.gamma * list_of_discounted_cumsums[T-i]
