...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss = -torch.mul(self(observations).log_prob(actions),advantages).sum() / batch_size
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        train_log = {
            'Policy Training Loss': ptu.to_numpy(loss),
        }
        if self.nn_baseline:
            ...
            q_values = utils.normalize_batch(q_values)
            q_values = ptu.from_numpy(q_values).unsqueeze(1)
            pred_baseline = self.baseline(observations)
            assert pred_baseline.shape == q_values.shape, f"{pred_baseline.shape}, {q_values.shape}"
            baseline_loss = nn.MSELoss()(input=pred_baseline, target=q_values)
            self.baseline_optimizer.zero_grad()
            baseline_loss.backward()
            self.baseline_optimizer.step()
            train_log.update({"Baseline Training Loss": ptu.to_numpy(baseline_loss)})
        return train_log
