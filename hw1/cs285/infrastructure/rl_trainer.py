...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:
            #load training data from expert data
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            #path[i]: {"obs":[],...}
            for j in range(utils.get_pathlength(paths[i])):
                paths[i]['action'][j] = expert_policy.get_action(paths[i]['observation'][j])#see ..policy.py
        return paths
