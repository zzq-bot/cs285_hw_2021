...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        qa_t_values = self.q_net(ob_no)#batch_size * ac_num
        assert qa_t_values.shape == (batch_size, self.ac_dim), f"{qa_t_values.shape}"
        assert ac_na.shape == (batch_size,)
        q_t_values = torch.gather(qa_t_values, 1, ac_na.unsqueeze(1)).squeeze(1)
        assert q_t_values.shape == (batch_size, )
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        #hw5.pdf mistakes the bracket, can see the paper for detailed equation.
