...
class MLPPolicyAWAC(MLPPolicy):
    ...
    def update(self, observations, actions, adv_n=None):
        ...
        logprob = -action_distribution.log_prob(actions)
        adv_n = torch.exp(1.0 / self.lambda_awac * adv_n).detach()#doesnt have to detach, only relate params in critic
        actor_loss = torch.mean(torch.mul(logprob, adv_n))
